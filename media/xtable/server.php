...
if (isset($conditions['codigo']) && strlen(trim($conditions['codigo'])) > 0) {
    $where .= $strAnd . " codigo = '" . mysql_real_escape_string($conditions['codigo']) . "' ";
    $strAnd = " AND ";
}
...
$sql = 'SELECT SQL_CALC_FOUND_ROWS codigo,nur,referencia,estado FROM documentos ' . $where . $order . $limit;
...
while ($row = mysql_fetch_object($result)) {
    $data[] = array(
        'codigo' => '<b>'.utf8_encode($row->codigo).'</b>',
        'nur' => utf8_encode($row->nur),
