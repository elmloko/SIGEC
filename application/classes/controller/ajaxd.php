...
class Controller_Ajaxd extends Controller
{
    ...
    public function action_documentosjson($user)
    {
        ...
        $esql = "   SELECT 
                        ...
                        d.nur,
                        d.cite_original,
                        DATE_FORMAT(d.fecha_creacion, '%d/%m/%Y %H:%i:%s') AS fecha,
                        fecha_creacion,
                        t.tipo,
                        d.original
                    FROM
                        documentos d USE INDEX (INDEX_ID_USER__FECHA_CREACION)
                            INNER JOIN
                        tipos t USE KEY (PRIMARY) ON d.id_tipo = t.id
                    WHERE
                        d.id_user = '$user'
        ...
        foreach ($result as $row) {
            ...
            if (strlen($row['nur']) > 1 && $row['estado'] == 0) {
                ...
                // === VER SI HOJA DE RUTA TIENE DERIVACIONES PENDIENTES POR EL USUARIO ===
                $nur = $row['nur'];
                ...
                // ========= [INICIO] VERIFICAMOS SI EL DOCUMENTO TIENE ARCHIVOS ADJUNTOS =========

                $archivos = ORM::factory('archivos')->where('id_documento', '=', $row['id'])->and_where('estado', '=', 1)->find_all();

                // ========= [FIN] VERIFICAMOS SI EL DOCUMENTO TIENE ARCHIVOS ADJUNTOS =========


                if (intval($nro_derivaciones) == 0 && count($archivos) > 0) {
                    ...
                    // --- Ver si esta en la Bandeja de Pendientes del Usuario ---
                    $query_nro_resultados = "  SELECT 
                    ...
                    $resultSet_nro_resultados = db::query(Database::SELECT, $query_nro_resultados, FALSE)
                        ->execute()
                        ->as_array();

                    $nro_resultados = $resultSet_nro_resultados[0]['nro_resultados'];

                    // ---------- [INICIO] VERIFICAR ARCHIVOS ADJUNTOS ----------
                    if (intval($nro_resultados) == 1) {
                        ...
                        if (count($archivos) == 0) {
                            ...
                            if (count($archivos) == 0 && $user == '95') {
                                $link = '<a href="/route/deriv/?hr=' . $row['nur'] . '" title="Derivar Hora de Ruta" class="text-xl text-warning"><i class="md md-play-circle-outline"></i></a>';
                                $link .= '<a href="/route/trace/?hr=' . $row['nur'] . '" title="Derivado: Ver seguimiento" class="text-xl text-success"><i class="md md- md-verified-user "></i></a>';
                                $link .= '<a href="/print/hr/?code=' . $row['nur'] . '" title="Imprimir hoja de ruta" class="text-xl text-primary"><i class="md md-print "></i></a>';
                            } else {
                                $link .= '<a href="#" onClick="alert(\'Usted debe subir el archivo adjunto.\')" title="Derivar Hoja de Ruta" class="text-xl text-warning"><i class="md md-play-circle-outline"></i></a>';
                            }
                        }
                    }
                    // ---------- [FIN] VERIFICAR ARCHIVOS ADJUNTOS ----------
                }
                // =======================================================
            ...
            $orders[] = array(
                ...
                'id' => $row['id'],
                'idd' => $row['id'],
                'nur' => '<a href="/route/trace/?hr=' . $row['nur'] . '" title="Derivar Hora de Ruta" class="text-primary-dark">' . $row['nur'] . '</a>',
    ...
    public function action_ventanillajsonp($user)
    {
        ...
        $esql = "SELECT d.id,CONCAT(d.nombre_destinatario,'<br><b>',d.cargo_destinatario,'</b>') as destinatario
                ,CONCAT(d.nombre_remitente,'<br><b>',d.cargo_remitente,'<br><span class=\"text-primary-dark\">',d.institucion_remitente,'</span></b>') as remitente,d.estado,
                d.referencia,d.nur,d.cite_original,DATE_FORMAT(d.fecha_creacion,'%d/%m/%Y %H:%i:%s') as fecha,fecha_creacion,d.original
                FROM documentos d USE INDEX (INDEX_ID_USER , INDEX_ESTADO)
