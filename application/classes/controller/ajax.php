...
//nuevo
    ...
    public function action_derivar()
    {
        if (isset($_POST['tipo'])) {
            ...
            $destino = DB::select('*')->from("users")->where('id', '=', $id_destino)->as_object()->execute()->current();
            $oficina_destino = ORM::factory('oficinas', $destino->id_oficina);
            //$remite = ORM::factory('users', $user);
            $remite = DB::select('*')->from("users")->where('id', '=', $id_user)->as_object()->execute()->current();
...
//alertas
            ...
            $result = array(
                ...
                'POST[oficial]' => $_POST['oficial'],
                'POST[id_seg]' => $_POST['id_seg'],
                'POST[destino]' => $_POST['destino'],
                'POST[accion]' => $_POST['accion'],
                'POST[proveido]' => $_POST['proveido'],
                'POST[nur]' => $_POST['nur'],
                'POST[fecha]' => $_POST['fecha'],
                'POST[user]' => $_POST['user'],
                'POST[document]' => $_POST['document'],
                'POST[hijo]' => $_POST['hijo'],
                'POST[urgente]' => $_POST['urgente'],
                // ORM Seguimiento
                'seguimiento->id_seguimiento' => $id_seg,
                'seguimiento->nur' => $nur,
                'seguimiento->derivado_por' => $remite->id,
            ...
            $mail->Host = "mail.correos.gob.bo";
            $mail->Port = 25;
            $mail->Username = "info@correos.gob.bo";
            $mail->Password = "123456";
            $mail->SMTPSecure = 'tls';
            $mail->SetFrom('info@correos.gob.bo', 'SIGEC');
