...
class Model_Users extends ORM {
    ...
    public function conectados() {
        $sql = "SELECT COUNT(*) as cantidad FROM (
                SELECT DISTINCT(user_id),session,(unix_timestamp()-last_active)/60 as minutos,from_unixtime(last_active) AS last_active  FROM sigec.sesiones) AS x
                WHERE x.minutos < 10";
        return db::query(Database::SELECT, $sql)->execute();
    }
    //usuarios conectados
