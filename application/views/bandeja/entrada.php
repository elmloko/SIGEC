...
<?php if (sizeof($entrada) > 0) { ?>
    <div style=";position: fixed; z-index: 1; margin-top: -24px; padding: 2px; background: #fff; width: 100%;"
         class="col-lg-12 col-md-12">
        <div class="col-md-5">
            <h3><i class="md md-inbox"></i> Correspondencia Entrante</h3>
        </div>
        ...
        <div class="col-md-4">
            <div class="btn-group">
                <button class="btn ink-reaction btn-sm btn-default" type="button"><i class=" fa fa-sort-alpha-asc"></i>
                    Ordenar por
                </button>
                <button data-toggle="dropdown" class="btn ink-reaction btn-sm btn-default dropdown-toggle" type="button"
        ...
        <div class="col-lg-4 ">
            <div id="lateralddf">
                ...
                <div id="print_enviados" class="oculto archive">
                    <ul>
                        <li>
                            <a href="#" id="print_hr"><img src="/media/images/excel.png" align="absmiddle"/><b> Generar
                                    Excel</b></a>
                        </li>
                    </ul>
    ...
    <div id="entrada" class="card" style="margin-top: 42px; position: relative;">
        <form action="/correspondence/doa" method="post" id="doa">
            ...
            foreach ($entrada as $s):
                ?>
                <div id="e<?php echo $i; ?>" class="bandeja tipo<?php echo $s->oficial; ?>"
                     style="display:inline-block; " oficina="<?php echo $s->de_oficina ?>"
                     proceso="<?php echo $s->referencia ?>" fecha="<?php echo $s->fecha; ?>"
                     hojaruta="<?php echo $s->nur; ?>">
                    <table class="oficial<?php echo $s->oficial; ?> ">
                        ...
                        <tr>
                            ...
                            <td class="derecha" valign="top">
                                ...
                                <?php if ($s->hijo == 1): ?> <a
                                        href="/correspondence/agrupado/?hr=<?php echo $s->nur; ?>"
                                        class="link agrupado">
                                        Agrupado</a><?php endif; ?>
                            </td>
                        </tr>
                        <tr>
                            <td width="88">
                            ...
                            <td>
                                <span class="opciones">
                                    ...
                                    <a href="javascript:;" entrada="<?php echo $i; ?>"
                                       class="recibir btn btn-primary-dark btn-sm" data-toggle="modal"
                                       data-target="#simpleModal" nur="<?php echo $s->id; ?>"
                                       hr="<?php echo $s->nur; ?>"><i class="md md-inbox"></i> Recibir</a>

                                    <!-- === OBSERVACION DE 'RECHAZO DE DERIVACION' === -->
                                    <?php
                                    $id_seguimiento = $s->id;
                                    $id_seguimiento_padre = $s->id_seguimiento;
                                    $nur = $s->nur;
                                    $id_usuario = $user->id;
                                    $estado = $s->estado;
                                    ...
                                    ?>
                                    <a href="#"
                                       class="btn btn-sm btn-primary-dark btn-modal-rechazo"
                                       data-toggle="modal"
                                       data-id-seguimiento="<?php echo $id_seguimiento; ?>"
                                       data-id-seguimiento-padre="<?php echo $id_seguimiento_padre; ?>"
                                       data-nur="<?php echo $nur; ?>"
                                       data-id-usuario="<?php echo $id_usuario; ?>"
                                       data-estado="<?php echo $estado; ?>"
                                       data-target="#modal-rechazo-<?php echo $id_seguimiento; ?>"
                                        <i class="fa fa-share"></i>RECHAZAR
                                    </a>

                                    <!-- Modal -->
                                    <div class="modal fade" id="modal-rechazo-<?php echo $id_seguimiento; ?>"
                                         role="dialog">
                                        <div class="modal-dialog">
                                            ...
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    ...
                                                    <p>
                                                        Ingrese una justificación, por la cual desea rechazar la
                                                        derivación.
                                                    </p>
                                                    <textarea style="height: 100px; width: 100%;" type="text"
                                                              class="form-control texto-rechazo" name="texto-rechazo"
                                                              id="texto-rechazo-<?php echo $id_seguimiento; ?>"
                                                              size="15"
                                                              placeholder="Ingrese su descripcion del rechazo..."></textarea>
                                                </div>
                                                <div class="modal-footer">
                                                    <button id="btn-insertar-rechazo" type="button"
                                                            class="btn btn-default" onclick="guardar_rechazo()">
                                                        GUARDAR
                                                    </button>
                                                    ...
                                                    <input type="hidden" class="id_seguimiento_padre"
                                                           value="<?php echo $id_seguimiento_padre; ?>"/>
                                                    <input type="hidden" class="nur" value="<?php echo $nur; ?>"/>
                                                    <input type="hidden" class="id_user"
                                                           value="<?php echo $id_usuario; ?>"/>
                                                    <input type="hidden" class="estado" value="<?php echo $estado; ?>"/>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <!-- === FIN OBSERVACION DE 'RECHAZO DE DERIVACION' === -->
...
<div class="modal fade" id="simpleModal" tabindex="-1" role="dialog" aria-labelledby="simpleModalLabel"
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Esta usted seguro de recibir la hoja de ruta: <span class="text-primary" id="hojaruta"></span>?</p>
                <input type="hidden" id="input-hojaruta" name="" value="0"/>
                <input type="hidden" id="input-entrada" name="" value="0"/>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-default btn-sm" data-dismiss="modal">Cancelar</button>
                <button type="button" id="btn-recibir" class=" btn btn-primary-dark btn-sm">Recibir</button>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal-dialog -->
</div><!-- /.modal -->
...
<script>
    $(document).ready(function () {
        $(".btn-modal-rechazo").click(function (event) {

            console.log('id-seguimiento:', $(this).data('id-seguimiento'));
            console.log('id-seguimiento-padre:', $(this).data('id-seguimiento-padre'));
            ...
            var id_seguimiento_padre = $(this).data('id-seguimiento-padre');
            var nur = $(this).data('nur');
            var id_usuario = $(this).data('id-usuario');
            var estado = $(this).data('estado');

            $(".id_seguimiento").val(id_seguimiento);
            $(".id_seguimiento_padre").val(id_seguimiento_padre);
            $(".nur").val(nur);
            ...
            $('#modal-rechazo').appendTo("body").modal('show');
        });
    });

    function guardar_rechazo() {
        var id_seguimiento = $('.id_seguimiento').val();
        var id_seguimiento_padre = $('.id_seguimiento_padre').val();
        var nur = $('.nur').val();
        var observacion = $('#texto-rechazo-' + id_seguimiento).val();
        var id_user = $('.id_user').val();
        var estado = $('.estado').val();

        console.log('id_seguimiento:', id_seguimiento);
        console.log('id_seguimiento_padre:', id_seguimiento_padre);
        console.log('nur:', nur);
        ...
        if (observacion.length > 0) {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    id_seguimiento_padre: id_seguimiento_padre,
                    nur: nur,
