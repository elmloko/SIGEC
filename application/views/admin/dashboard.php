<script>
    ...
    $(document).ready(function () {
        ...
        // Bitacora: paginada, se recarga solo al cambiar de pagina (no en el polling de 5s)
        function loadBitacora(page) {
            page = page || 1;
            $.getJSON('/admin/ajax/bitacora', {page: page, limit: 15}, function (resp) {
                $('table#bitacora tbody tr').remove();
                $.each(resp.data, function (i, e) {
                    $('table#bitacora tbody').append('<tr><td>' + e.fecha + '</td><td>' + (e.usuario || '-') + '</td><td>' + e.accion_realizada + '</td><td>' + e.ip + '</tr>');
                });
                $('#bitacora-total').text(resp.total);
                renderBitacoraPager(resp.page, resp.pages);
            });
        }

        function renderBitacoraPager(page, pages) {
            var $pager = $('#bitacora-pagination');
            $pager.empty();
            if (pages <= 1) {
                return;
            }
            ...
            function addItem(label, targetPage, disabled, active) {
                var $li = $('<li>').toggleClass('disabled', !!disabled).toggleClass('active', !!active);
                var $a = $('<a href="#">').text(label);
                if (!disabled && !active) {
                    $a.on('click', function (ev) {
                        ev.preventDefault();
                        loadBitacora(targetPage);
                    });
                }
                $li.append($a);
                $pager.append($li);
...
<div class="row">
    ...
    <div class=" col-sm-12 col-md-6">
        <div class="card style-default-bright no-padding card-underline">
            <div class="card-head">
                <header>Actividad Reciente: <small>(<span id="bitacora-total">0</span> registros)</small></header>
            </div>
            ...
            <div class="card-body no-padding text-center" style="padding: 5px 0;">
                <ul class="pagination pagination-sm no-margin" id="bitacora-pagination"></ul>
            </div>
